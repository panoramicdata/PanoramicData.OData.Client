...
Write-BuildMessage "Version: $version"
...
Write-BuildMessage "Tag $version pushed."
...
Write-BuildMessage "Waiting for the release run for $version..."
...
Write-BuildMessage "Run: https://github.com/$repoFullName/actions/runs/$runId"
gh run watch $runId --repo $repoFullName --exit-status --interval 20
$runExitCode = $LASTEXITCODE

if ($runExitCode -ne 0) {
	Write-BuildMessage ""
	Write-BuildMessage "The release run did not succeed: https://github.com/$repoFullName/actions/runs/$runId" -ForegroundColor Red
	...
	if ($LASTEXITCODE -eq 0 -and $jobId) {
		...
		if ($LASTEXITCODE -eq 0 -and $annotation) {
			Write-BuildMessage "Reason: $annotation" -ForegroundColor Red
		}
	}

	Write-BuildMessage ""
	Write-BuildMessage "Tag $version is pushed but no package was published. Once the cause is fixed:" -ForegroundColor Yellow
	Write-BuildMessage "  gh run rerun $runId --repo $repoFullName --failed" -ForegroundColor Cyan
	exit 1
}

Write-BuildMessage "Package $version published." -ForegroundColor Green